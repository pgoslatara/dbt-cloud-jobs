...
def test_create_dbt_cloud_job(file_job_minimal_definition) -> None:
    ...
    assert definition["name"] in [
        x["name"]
        for x in list_dbt_cloud_jobs(definition["account_id"], project_id=definition["project_id"])
    ]
...
def test_delete_dbt_cloud_job(file_job_minimal_definition) -> None:
    ...
    existing_jobs = list_dbt_cloud_jobs(
        definition["account_id"], project_id=definition["project_id"]
    )
    delete_dbt_cloud_job(
...
def test_list_dbt_cloud_jobs() -> None:
    existing_jobs = list_dbt_cloud_jobs(
        int(os.environ["DBT_ACCOUNT_ID"]), project_id=int(os.environ["DBT_PROJECT_ID"])
    )
